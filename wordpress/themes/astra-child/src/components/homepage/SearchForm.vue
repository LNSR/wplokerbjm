...
import CustomDropdown from './SearchForm/CustomDropdown.vue'
...
function removeFilter(key: keyof SearchFilters, value: string) {
  const arr = Array.isArray(searchStore.filters[key]) ? [...(searchStore.filters[key] as string[])] : []
  const idx = arr.indexOf(value)
  if (idx !== -1) {
    arr.splice(idx, 1)
    // @ts-expect-error: Only lokasi/gender/pendidikan are arrays
...
onMounted(() => {
  ...
  const el = document.getElementById('search-form-parent')
  if (el) el.classList.remove('hide-preload')
})
</script>

<template>
  <form :class="{ 'invisible': !hydrated }" class="space-y-4" :action="archiveLink" method="get"
    ...
    <div class="flex gap-2 relative">
      ...
      <button type="submit" class="btn btn-primary rounded-l-none px-4"
        ...
        <i class="fas" :class="(searchStore.loading || taxonomyStore.loading) ? 'fa-spinner fa-spin' : 'fa-search'"></i>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <div class="relative">
        ...
        <CustomDropdown v-model="searchStore.filters.lokasi"
          :options="mapTerms(taxonomyStore.lokasiTerms, 'Semua lokasi')" placeholder="Semua Lokasi"
          :multiple="true" :disabled="searchStore.loading || taxonomyStore.lokasiLoading"
          @open="taxonomyStore.fetchLokasiTerms()" />
      </div>
      <!-- Gender -->
      <div class="relative">
        <i
          class="fas fa-venus-mars absolute left-3 top-1/2 -translate-y-1/2 text-blue-500 pointer-events-none z-10"></i>
        <CustomDropdown v-model="searchStore.filters.gender"
          :options="mapTerms(taxonomyStore.genderTerms, 'Semua gender')" placeholder="Semua Gender"
          :multiple="true" :disabled="searchStore.loading || taxonomyStore.genderLoading"
          @open="taxonomyStore.fetchGenderTerms()" />
      </div>
      <!-- Pendidikan -->
      <div class="relative">
        <i
          class="fas fa-graduation-cap absolute left-3 top-1/2 -translate-y-1/2 text-blue-500 pointer-events-none z-10"></i>
        <CustomDropdown v-model="searchStore.filters.pendidikan"
      ...
      <div class="relative">
        ...
        <CustomDropdown v-model="searchStore.filters.sort" :options="sortOptions" placeholder="Urutkan"
          :multiple="false" :disabled="searchStore.loading || taxonomyStore.loading" />
      </div>
    </div>
        ...
        <template v-for="(val, idx) in filter.values" :key="val">
          <span
            ...
            <button type="button" class="!ml-2 text-blue-500 hover:text-red-600 transition-colors duration-150"
              @click="removeFilter(filter.key as keyof SearchFilters, val)" aria-label="Hapus filter">
    ...
    <!-- Loading State -->
    <div v-if="searchStore.loading || taxonomyStore.loading" class="text-center py-4 flex flex-col items-center justify-center">
      ...
      <span v-if="taxonomyStore.loading" class="!pl-2">Memuat data...</span>
      <span v-else-if="searchStore.suggestionsLoading" class="!pl-2">Mencari saran...</span>
