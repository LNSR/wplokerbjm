<template>
  <div class="relative" tabindex="0" @blur="close" ref="dropdownRef">
    <button type="button"
      ...
      @click="toggle">
      <span class="!pl-6">{{ multiSelectLabel || placeholder }}</span>
      <i class="fas fa-chevron-right float-right !mt-1 transition-transform" :class="{ 'rotate-90': open }"></i>
    </button>
    <ul v-show="open"
      ...
      <!-- Options list -->
      <li v-for="(option, idx) in filteredOptions.filter(opt => opt.value !== '')" :key="option.__key" :class="[
      ...
      <li v-if="filteredOptions.length === 0" class="!px-5 !py-2 text-gray-400 text-center">Tidak ada hasil</li>
    </ul>
  </div>
</template>
...
const {
  open, activeIndex, search, breadcrumb, selectedValues, multiSelectLabel,
  isSelected, select, toggleValue, isMultiple, SEMUA_VALUE, toggle, close,
  goBack, navigateChildren, goToBreadcrumb, filteredOptions, highlightMatch
} = useDropdown({
