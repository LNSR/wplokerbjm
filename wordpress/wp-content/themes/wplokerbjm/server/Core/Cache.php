...
class TransientCache
{
    ...
    public static function set($key, $value, $expiration = 0)
    {
        try {
            ...
            error_log('TransientCache::set error: ' . $e->getMessage());
            return false;
        }
    }
    ...
    public static function get($key): mixed
    {
        try {
            ...
            error_log('TransientCache::get error: ' . $e->getMessage());
            return false;
        }
    }
    ...
    public static function delete($key): bool
    {
        try {
            ...
            error_log('TransientCache::delete error: ' . $e->getMessage());
            return false;
        }
    }
    ...
    public static function deletePattern($pattern): int
    {
        try {
            ...
            error_log('TransientCache::deletePattern error: ' . $e->getMessage());
            return 0;
        }
    }
    ...
    public static function incrementTransient($key, $value = 1, $expiration = 0): int|false
    {
        ...
        try {
            ...
            error_log('TransientCache increment fallback failed: ' . $e->getMessage());
            return false;
        }
    }
    ...
    public static function decrementTransient($key, $value = 1): int|false
    {
        ...
        try {
            ...
            error_log('TransientCache decrement fallback failed: ' . $e->getMessage());
            return false;
        }
    }
}
    ...
    public static function increment($key, $value = 1, $expiration = 0): int|false
    {
        try {
            if (!function_exists('wp_cache_incr')) {
                return TransientCache::incrementTransient($key, $value, $expiration);
            }
    ...
    public static function decrement($key, $value = 1): int|false
    {
        try {
            if (!function_exists('wp_cache_decr')) {
                return TransientCache::decrementTransient($key, $value);
            }
            $current = self::get($key);
            if ($current === false) {
                self::set($key, 0, 0);
