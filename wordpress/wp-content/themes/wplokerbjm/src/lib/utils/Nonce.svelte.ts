...
class NonceManager {
  ...
  private loggedInNonce = $derived(themeManager.getThemeData?.wpRestNonce as WPLokerBJMThemedData["wpRestNonce"]); // check initial nonce existence to determine if user is logged in

  /**
   * Synchronously reads the nonce from the cached theme data props.
   */
  private get readStorage(): WPLokerBJMThemedData["wpRestNonce"] {
