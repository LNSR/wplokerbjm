...
<script lang="ts">
  ...
  async function createSwiper(): Promise<void> {
    ...
    if (!cssLoaded) {
      try {
        ...
        console.warn("Failed to dynamically load Swiper CSS:", e);
      }
    }
    ...
    while (true) {
      ...
      if (slidesCount > 0 && width > 0) break;
      attempts += 1;
    ...
    try {
      ...
      const SwiperCtor: typeof SwiperCore = SwiperCore;
      const cfg = createSwiperConfig(paginationEl, nextEl, prevEl);
      const finalCfg = {
        ...(cfg || {}),
        modules: [Navigation, Pagination, Autoplay],
      };
      try {
        swiperInstance = new SwiperCtor(el, finalCfg);
      } catch (staticErr) {
        console.warn(
          "Static Swiper constructor failed; falling back to dynamic import.",
          String(staticErr)
        );
        console.debug(
          "Static Swiper constructor failed, falling back to dynamic import:",
        ...
        try {
          const [SwiperModule, Modules] = await Promise.all([
            import("swiper"),
            import("swiper/modules"),
          ]);
          const SwiperLib = (SwiperModule &&
            (SwiperModule.default ?? SwiperModule)) as typeof SwiperCore;
          const {
            Navigation: DynNav,
            Pagination: DynPag,
            Autoplay: DynAuto,
          } = Modules as typeof import("swiper/modules");
          if (SwiperLib && SwiperLib.use) {
            SwiperLib.use([DynNav, DynPag, DynAuto]);
          }
          const cfg2 = createSwiperConfig(paginationEl, nextEl, prevEl);
          swiperInstance = new SwiperLib(el, {
            ...(cfg2 || {}),
            modules: [DynNav, DynPag, DynAuto],
          });
        } catch (dynErr) {
          console.error(
            "Dynamic import fallback also failed to initialize Swiper:",
            dynErr
          );
          throw dynErr;
        }
  ...
  onMount(() => {
    // Register Swiper modules and attempt a best-effort initialization on mount.
    // createSwiper will also register modules as a safeguard.
    try {
      if (
        (SwiperCore as typeof SwiperCore) &&
        (SwiperCore as typeof SwiperCore).use
      ) {
        (SwiperCore as typeof SwiperCore).use([
          Navigation,
          Pagination,
    ...
    // Kick off a best-effort init — reactive effects will re-run createSwiper
    // when job data arrives or changes.
    requestAnimationFrame(() => void createSwiper());
  });
  ...
  $effect(() => {
    ...
    if (!swiperInstance) {
      ...
      requestAnimationFrame(() => createSwiper());
      return;
    }
    ...
    requestAnimationFrame(() => reinitializeSwiper());
  });

  // Destroy and recreate the swiper instance (used by refresh button)
  async function reinitializeSwiper({ forceDestroy = false } = {}) {
    ...
    const el = swiperContainerEl as HTMLElement | null;
    if (!el) {
      // Nothing to initialize; ensure any lingering "invisible" class is
      // removed on any DOM node we can reach (best-effort) so the UI doesn't
  ...
  async function refreshCarousel() {
    ...
    await tick();
    await reinitializeSwiper({ forceDestroy: true });
  }

  onDestroy(() => {
    if (swiperInstance) swiperInstance.destroy(true, true);
    swiperInstance = null;
    // cleanup complete — no observers or retry timers to clear
    // Clear DOM refs so subsequent mounts get fresh elements
      ...
      {#if swiperFailed}
        <button
          ...
          onclick={() => {
            ...
            swiperFailed = false;
            void reinitializeSwiper({ forceDestroy: true });
          }}
        >
          Coba lagi
        </button>
...
<style>
  /* When Swiper fails, apply layout fallback and hide native controls */
  /* When Swiper fails, apply layout fallback and hide native controls */
