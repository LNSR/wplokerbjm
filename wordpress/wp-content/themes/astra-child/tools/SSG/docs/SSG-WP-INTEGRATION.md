...
- `PostsCRUDListener` — listens to post create/update/delete and enqueues or resolves affected URLs.
  - Implementation: `inc/Services/PostsManagement/SSG/PostsCRUDListener.php`
...
- `TriggerBuild` — orchestrates dispatching builds to GitHub Actions (selective or full-site). See `inc/Services/PostsManagement/SSG/TriggerBuild.php`.
- `PathResolver` — helper that normalizes and deduplicates paths before dispatch (usually part of the services in `inc/Services/Utilities/SSG/`).
...
- `DispatchSSGBuild` — REST controller for manual SSG dispatches, validates input and permissions, filters paths, and calls `TriggerBuild`.
  - Implementation: `inc/Controllers/REST/DispatchSSGBuild.php`
  - REST route registration: `inc/Services/REST/RESTRoute.php` (registers the `/astra-child/v1/dispatch-ssg/` endpoint and connects it to `DispatchSSGBuild::handle`).

### 4. Configuration & Utilities

- `URLFilterService` — filters and sanitizes URL lists before dispatch (used by `DispatchSSGBuild`): `inc/Services/Utilities/SSG/URLFilterService.php`.

Quick navigation (click to open the files in your editor):

- `inc/Services/PostsManagement/SSG/PostsCRUDListener.php` — post events → path collection
- `inc/Services/PostsManagement/SSG/TriggerBuild.php` — triggers GitHub Actions dispatch
- `inc/Controllers/REST/DispatchSSGBuild.php` — REST handler (permission/rate-limit/dry-run)
- `inc/Services/REST/RESTRoute.php` — registers REST routes used by SSG tools
- `inc/Services/Utilities/SSG/URLFilterService.php` — path filtering and normalization

If any of these files are missing in your local tree, they live under `inc/Services/...` and are part of the SSG integration layer.
