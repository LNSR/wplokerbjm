...
<article class="bg-white rounded-lg shadow-lg p-8 divide-y divide-gray-200">
    ...
    <section class="pt-8 group">
        <?php if (!JobHelpers::isReallyEmpty($job->getAttribute('company'))) : ?>
    ...
    <?php 
    // Initialize the share button component once
    $share_button = new \AstraChild\Views\Components\ShareButton();
    $has_contact_info = $job->hasContactInfo();
    $has_social_media = !JobHelpers::isReallyEmpty($job->getAttribute('socials')) && is_array($job->getAttribute('socials'));
    ?>
    ...
    <?php if ($has_contact_info) : ?>
        <section class="pt-8">
            <h3 class="flex items-center justify-between gap-2 text-xl font-semibold text-gray-800 mb-6">
                ...
                <!-- share button - shown in contact section if it exists -->
                <?php $share_button->render($job); ?>
            </h3>
    ...
    <?php if ($has_social_media) : ?>
        <section class="pt-8">
            <h3 class="flex items-center justify-between gap-2 text-xl font-semibold text-gray-800 mb-6">
                ...
                <!-- share button - only shown here if contact section doesn't exist -->
                <?php if (!$has_contact_info) : ?>
                    <?php $share_button->render($job); ?>
                <?php endif; ?>
            </h3>
