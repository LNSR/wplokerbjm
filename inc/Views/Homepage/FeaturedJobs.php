...
class FeaturedJobs
{
     ...
     * @var LoadMore
     */
    protected $loadMore; // Changed variable name
    ...
    public function render(array $options = []): void
    {
        $default_options = [
            ...
            'show_load_more' => true, // Rename from show_pagination
            'grid_gap' => 'gap-6',
            'filter_jobs' => [
                'show_statuses' => [
        ...
        <section class="featured-jobs-section mb-12">
            <div class="<?php echo esc_attr($options['max_width'] . ' ' . $options['margin_x']); ?>">
                ...
                <?php if ($options['show_load_more']): ?>
                    <?php 
                    // Change this: Pass better data attributes instead of just a function name
                    $this->loadMore->render(
                        $featured_jobs,
                        'featured-jobs',
                        null // Remove JavaScript function name, we'll use data attributes instead
                    ); 
                    ?>
