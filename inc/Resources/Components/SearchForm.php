...
?>
	...
	private static function render_hierarchical_options($terms, $current_selected = '', $taxonomy = '', $depth = 0)
	{
		ob_start();
		foreach ($terms as $term) {
			$indent = str_repeat('—', $depth) . ($depth > 0 ? ' ' : '');
		?>
			<option value="<?= esc_attr($term->slug); ?>" <?= selected($current_selected, $term->slug, false); ?>>
				<?= $indent . esc_html($term->name); ?>
			</option>
<?php

			// Get child terms for this taxonomy
			...
			if (! empty($child_terms) && ! is_wp_error($child_terms)) {
				echo self::render_hierarchical_options($child_terms, $current_selected, $taxonomy, $depth + 1);
			}
		}
		return ob_get_clean();
	}
