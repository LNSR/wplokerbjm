...
class SocialMedia
{
    ...
    public static function getLinkData(string $platform, string $username): ?array
    {
        ...
        if ($platform === 'Whatsapp') {
            ...
            // Handle LinkedIn - check if it's a company profile
            if (strpos(strtolower($username), 'company/') !== false || strpos(strtolower($username), 'company:') === 0) {
                // It's a company profile
                $clean_username = str_replace('company:', '', strtolower($username));
                $clean_username = str_replace('company/', '', $clean_username);
                $clean_username = ltrim($clean_username, '/');
                $clean_username = ltrim($clean_username, '@');

                return [
                    'icon' => $config['icon'],
                    'url' => 'https://linkedin.com/company/' . $clean_username,
                    'username' => $username // Keep original format
                ];
            }
            
            // Otherwise treat as personal profile
